        ...
        stage ('BuildImage') {
            ...
            // this is the list of commands that will be run in the agent
              sh '''
                ...
                echo "tagging images with:${REGISTRY_HOST}/mra/ngrefarch/mra-pages/${NG_BRANCH}"
                docker build -t ${REGISTRY_HOST}/mra/ngrefarch/mra-pages/${NG_BRANCH}/mra-pages:${BUILD_NUMBER} .
                docker push ${REGISTRY_HOST}/mra/ngrefarch/mra-pages/${NG_BRANCH}/mra-pages:${BUILD_NUMBER}
          ...
          script {
            if (currentBuild.getPreviousBuild() &&
              currentBuild.getPreviousBuild().getResult().toString() != "SUCCESS") {
                emailext body: "MRA Pages Recovery: ${env.BUILD_URL}", recipientProviders: [[$class: 'DevelopersRecipientProvider'], [$class: 'RequesterRecipientProvider']], subject: "BUILD ERROR:${env.BRANCH_NAME}"
            }
          }
        }
        failure {
            emailext body: "MRA Pages Error on: ${env.BUILD_URL}", recipientProviders: [[$class: 'DevelopersRecipientProvider'], [$class: 'RequesterRecipientProvider']], subject: "BUILD ERROR:${env.BRANCH_NAME}"
        }
        unstable {
